...
a.setForeground(Color.WHITE);
    ...
    d.setForeground(Color.WHITE);
     ...
     a.setBackground(new Color(17, 41, 63));  
    b.setBackground(new Color(17, 41, 63));  
    c.setBackground(new Color(17, 41, 63));  
    d.setBackground(new Color(17, 41, 63)); 
    
    scrollPane.setPreferredSize(new Dimension(200,225));
    
Map<String, Integer> rankingNacionalidades = seleccionDAO.getRankingNacionalidadesDirectoresTecnicos();

int cantidadNacionalidades = seleccionDAO.getCantidadNacionalidadesDirectoresTecnicos();
String resultado = seleccionDAO.imprimirSeleccionesPorContinente();
int numeroSelecciones = seleccionDAO.getNumeroSelecciones();
      
for (String nacionalidad : rankingNacionalidades.keySet()) {
    int conteo = rankingNacionalidades.get(nacionalidad);
    d.append(nacionalidad + ": " + conteo + "\n");
}



            a.setText( "total de selecciones Cargadas: " + numeroSelecciones + "\n" );
            b.setText(" Cantidad de nacionalidades diferentes de los directores técnicos: " + cantidadNacionalidades+ "\n");
            c.setText(""+resultado );
         ...
         */
        private void accionDashboardRes() throws Exception {
       ...
       JScrollPane scrollPane = new JScrollPane(h);
        jPanelMain.setBackground(new Color(18, 119, 217));
    ...
    c.setBackground(new Color(17, 41, 63));  
    d.setBackground(new Color(17, 41, 63));  
    e.setBackground(new Color(17, 41, 63));  
    f.setBackground(new Color(17, 41, 63));  
    g.setBackground(new Color(17, 41, 63));  
    h.setBackground(new Color(17, 41, 63));  
    scrollPane.setPreferredSize(new Dimension(200,225));
    
    
         ResultadosDAO resultadosDAO = new ResultadosDAO();
         double promedioGolesPorPartido = resultadosDAO.getPromedioGolesPorPartido();
         int numeroPartidosCargados = resultadosDAO.getNumeroPartidosCargados();
         List<String> partidosGolesMenosMas = resultadosDAO.GetPardinosMasMenosGol();
         List<String> estadisticasPartidos = resultadosDAO.obtenerGyE();  
         List<String> resultados = resultadosDAO.getSeleccionesEMasGolesMenosG();
         List<String> resultadosMYMP = resultadosDAO.getSeleccionConMasYPuntosMenosP();
 List<String> ContinentesConMasYGolesMenosG = resultadosDAO.getContinentesConMasYGolesMenosG();

 Map<String, List<String>> clasificados = resultadosDAO.getClasificadosPorGrupo();
       
for (Map.Entry<String, List<String>> entry : clasificados.entrySet()) {
    String grupo = entry.getKey();
    List<String> equipos = entry.getValue();
     
    h.append("Clasificados del Grupo " + grupo + "\n");
    for (String equipo : equipos) {
         h.append("- " + equipo+"\n");
    }
}
 for (String resultado : ContinentesConMasYGolesMenosG) {
        g.append(resultado  + "\n");

}
      for (String resultado : resultadosMYMP) {
    f.append(resultado  + "\n");
}   
...
for (String resultado : partidosGolesMenosMas) {
    
    c.append(resultado  + "\n");
}
          
for (String resultado : estadisticasPartidos) {
    d.append(resultado  + "\n");;
}


            a.setText("Número de partidos cargados: " + numeroPartidosCargados);
            b.setText("Promedio de goles por partido: " + promedioGolesPorPartido);
        ...
        public void pintarTablaSelecciones() {
            ...
            form.add(label);
       String textoBusqueda = label.getText();
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        Seleccion seleccionEncontrada = seleccionDAO.buscarSeleccionPorNombre(textoBusqueda);

        // Actualizar el modelo de la tabla para mostrar solo los resultados de la búsqueda
        DefaultTableModel model = (DefaultTableModel) table.getModel();
        model.setRowCount(0); // Eliminar filas existentes en la tabla

        if (seleccionEncontrada != null) {
            model.addRow(new Object[]{seleccionEncontrada.getNombre(), seleccionEncontrada.getContinente(),
                seleccionEncontrada.getDt(), seleccionEncontrada.getNacionalidad()});
        }
    }
});
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        private void pintarPanelDerecho() {
            ...
            // Define las dimensoines del panel
